...
if ('/tmp/pear' != '@'.'WEB-DIR@')
{
    ...
    */
    ...
    require_once( '/tmp/pear' . PATH_DELIMITER . 'PhpDocumentor/docbuilder/includes/utilities.php' );
    ...
    $ini = phpDocumentor_parse_ini_file('/tmp/pear' . PATH_DELIMITER . 'PhpDocumentor/phpDocumentor.ini', true);
    if (isset($ini['_phpDocumentor_options']['userdir'])) {
        $configdir = $ini['_phpDocumentor_options']['userdir'];
    } else {
        $configdir = '/tmp/pear' . PATH_DELIMITER . 'PhpDocumentor/user';
    }
} else {
